...
function SearchBar({ setSearchQuery, onSearch }) {
  ...
  const handleGoClick = () => {
    if (matches.length > 0) {
      onSearch(matches[0].name); // Use the first match's name for the "Go!" action
    }
  };
...
async function fetchMatches(searchQuery) {
  try {
    ...
    const filteredMatches = data.results.filter(pokemon => pokemon.name.toLowerCase().includes(searchQuery.toLowerCase()));
